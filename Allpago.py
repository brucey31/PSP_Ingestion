...
cursor.execute("COPY allpago_raw2 FROM 's3://bibusuu/PSP_Reports/Allpago/'  CREDENTIALS 'aws_access_key_id=%s;aws_secret_access_key=%s';" % (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY))

print "Deleting old table Allpago_raw"
cursor.execute("drop table if exists Allpago_raw;")

print 'Renaming Allpago_raw2 to Allpago_raw'
cursor.execute("alter table Allpago_raw2 rename to Allpago_raw;")
