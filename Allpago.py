...
while start_date <= end_date:
    ...
    rs = check_output(["s3cmd", "ls", "s3://bibusuu/PSP_Reports/Allpago/%s" % start_date])
    ...
    if len(rs) > 1:
        ...
        for country in countries:
            ...
            call(["s3cmd", "put", '%s_Custom_report_BIP Extract_%s.csv' % (country, start_date),'s3://bibusuu/PSP_Reports/Allpago/%s/%s_Custom_report_BIP Extract_%s.csv' %(start_date, country, start_date)])
...
cursor.execute("create table allpago_raw as select unique_id,  email, ip,given_name, country, country_code, state, city,region_code, channel_name, reason_code_meaning, return_code_meaning, status_code_meaning, transaction_date, credit, debit, currency, payment_method, payment_type, fees, taxes, netpayout, bulk_id, reference_id, status_code, reason_code, return_code, transaction_id, account_last_four_digits, brand, account_year, account_month, bin_number, issuing_bank, card_type, card_level, boleto_due_date, num_installments from allpago_raw2;")
